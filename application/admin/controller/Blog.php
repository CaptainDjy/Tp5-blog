		...
		if (request()->isGet()) {
			...
			if ($file) {
				...
				if ($info) {
					...
					$this->error($file->getError(), url('admin/blog/add'));
				}
			}
			...
			if ($res) {
				...
				$this->error('添加失败', url('admin/blog/add'));
			}
		}
	}
		...
		if (request()->isGet()) {
			...
			if ($file) {
				...
				if ($info) {
					...
					$this->error($file->getError(), url('admin/blog/edit', ['id' => $id]));
				}
			}
			// print_r($data);exit;
			$c = BlogModel::get($id);
			...
			if ($res) {
				$this->success('保存成功', url('admin/blog/index'));
			} else {
				$this->error('保存失败', url('admin/blog/edit', ['id' => $id]));
			}
		}
	}
}
